...
2. `.xctestrun` ファイル内の `TestingEnvironmentVariables` ディクショナリに `SORA_SIGNALING_URL`、`TEST_SECRET_KEY`、`TEST_CHANNEL_ID_PREFIX`、`TEST_CHANNEL_ID_SUFFIX` を追加する（`plutil` で書き換え）
...
- ローカルの Xcode で `SORA_SIGNALING_URL` と `TEST_SECRET_KEY` をスキーム環境変数に設定して E2E テスト 3 ケースを実行し、すべて成功することを確認
...
- **環境変数未設定**: `SORA_SIGNALING_URL` が nil または空文字列の場合、`buildConfiguration()` が `XCTSkipUnless` でテストをスキップする。全テストケース（`testConnectRecvonly`、`testDisconnectRecvonly`、`testOfferAnswerCompleted`）がスキップ対象
- **`TEST_CHANNEL_ID_PREFIX` / `TEST_CHANNEL_ID_SUFFIX` が未設定**: prefix 未設定時は `""`、suffix 未設定時も `""` として扱い、channelId は `"e2e-test{github.run_id}"` のみで構成される
...
| 環境変数名 | 用途 | 種別 | デフォルト |
|---|---|---|---|
| `SORA_SIGNALING_URL` | Sora シグナリング接続先 URL | secret | なし（必須） |
| `TEST_SECRET_KEY` | `metadata.access_token` に設定する認証トークン | secret | なし（必須） |
| `TEST_CHANNEL_ID_PREFIX` | channelId の prefix | variable | `""` |
| `TEST_CHANNEL_ID_SUFFIX` | channelId の suffix | variable | `""` |

**channelId 構築ロジック**:

`TEST_CHANNEL_ID_PREFIX` と `TEST_CHANNEL_ID_SUFFIX` は省略可（未設定時は空文字列として扱う）。channelId は `{prefix}e2e-test{suffix}` の形式で組み立てる。suffix には CI 側で `_${{ github.run_id }}` が設定されるため一意性が担保される。ローカル実行時は suffix 未設定の場合 `e2e-test` のみとなる。
...
```swift
// SORA_SIGNALING_URL が未設定の場合はスキップ、不正な値の場合は失敗させる
guard
...
// audioEnabled / videoEnabled はデフォルト (true) のまま
// recvonly かつ両方 false はシグナリングエラーとなるため設定しない
```
...
加えて、`SignalingChannel.connect` (L213) では `Logger.info` レベルで `urlCandidates` を出力している。`SORA_SIGNALING_URL` がクエリパラメータに `access_token` を含む形式で渡される場合、`.info` レベルでも露出しうるため、E2E テストの `setUp()` で `Logger.shared.level = .warn` を設定し、`.info` 以下のログ出力を抑制する。
...
- `SORA_SIGNALING_URL` の実値をログ出力しない
...
- `SORA_SIGNALING_URL`、`TEST_SECRET_KEY`、`TEST_CHANNEL_ID_PREFIX`、`TEST_CHANNEL_ID_SUFFIX` を CI から注入できること
...
- `.github/workflows/ci.yml` (新規)
  - `e2e` job の追加（`runs-on: [self-hosted, macOS, ARM64, Apple-M1]`、Xcode 26.5、`xcodebuild test` 実行）
...
- `SoraTests/E2ETests.swift` (新規)
  - 環境変数 `SORA_SIGNALING_URL`、`TEST_SECRET_KEY`、`TEST_CHANNEL_ID_PREFIX`、`TEST_CHANNEL_ID_SUFFIX` の読み取りヘルパー
  - channelId 構築（`{prefix}e2e-test{suffix}`）
  - `testConnectRecvonly`、`testDisconnectRecvonly`、`testOfferAnswerCompleted` の実装
  - 環境変数未設定時の `XCTSkipUnless` スキップ
  - `Configuration` の組み立て（`role: .recvonly`、`signalingConnectMetadata` に access_token 注入）
- `Sora/Logger.swift`
