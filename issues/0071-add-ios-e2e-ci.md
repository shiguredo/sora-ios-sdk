...
`.github/workflows/build.yml` に E2E 専用の `e2e` job を追加する。既存の build job は GitHub-hosted runner のまま維持し、E2E のみを self-hosted runner に寄せる。
...
- 存在しない場合は `xcrun simctl create 'iPhone 16 Pro'` で作成する
- 起動は `xcrun simctl boot 'iPhone 16 Pro'` で行い、`xcrun simctl bootstatus 'iPhone 16 Pro' -b` で起動完了を待つ（起動完了前にテストが開始されるとエラーになるため）
- テスト実行後は `xcrun simctl shutdown 'iPhone 16 Pro'` で停止する（best effort）
...
- `Sora/SignalingChannel.swift`
  - `send(message:)` 内の `Logger.debug` 出力（L302-303）前に JSON 文字列の secret マスクを適用
  - `handle(message:)` 内のエラーログ出力（L339-341）でも同様にマスクを適用
  - マスク処理は private メソッドとして実装し、公開 API を増やさない
- `CHANGES.md`
