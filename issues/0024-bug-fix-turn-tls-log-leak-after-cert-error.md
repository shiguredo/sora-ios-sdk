...
2. `Configuration.webRTCConfiguration.iceServerInfos` に `turns:` の URL（例: `turns:<hostname>:443?transport=tcp`）を持つ `ICEServerInfo` を設定する（`ICEServerInfo.tlsSecurityPolicy` のデフォルトは `.secure` のため明示的な指定は不要）
...
```swift
// Sora/IOSCertificateVerifier.swift:39-53
...
接続失敗時の終了処理は `basicDisconnect`（`Sora/PeerChannel.swift:1251`）であり、その中の `nativeChannel?.close()`（`Sora/PeerChannel.swift:1280`）で `RTCPeerConnection` をクローズする。ネイティブのピア接続が閉じられず ICE 収集などが継続していると、ログが流れ続ける。

## 失敗経路の分析

TURN-TLS 証明書エラーが `PeerChannel` に伝わる経路として 2 つが考えられる:

**経路 A （WebSocket エラー経路）**: 証明書エラーが TURN ソケット失敗 → ICE 失敗 → WebRTC 内部エラー → シグナリング WebSocket が切断 → `signalingChannel.internalHandlers.onDisconnect`（`PeerChannel.swift:246`）→ `disconnect`（行 318）→ `basicDisconnect`（行 1251）

**経路 B （PeerConnectionState.failed 経路）**: 証明書エラー → TURN-TLS ハンドシェイク失敗 → ICE 状態が failed → `RTCPeerConnectionState.failed` → `peerConnection(_:didChange:RTCPeerConnectionState)`（行 1485）→ `disconnect`（行 318）→ `basicDisconnect`（行 1251）
...
`basicDisconnect`（`Sora/PeerChannel.swift:1251`）と `nativeChannel?.close()`（行 1280）にデバッグログを追加して実行し、証明書エラー発生後にこれらが呼ばれているかを確認する。

- `nativeChannel?.close()` が呼ばれていない場合: `disconnect` → `basicDisconnect` の経路のどこかで処理が止まっている。`Lock.waitDisconnect` のカウンタ（`PeerChannel.swift:96` の `private var count`）が 0 でない（別の非同期処理が `lock()` を保持している）可能性がある。または `disconnect` が `state == .closed` のため早期リターンしている可能性もある（行 319-321）。`Lock` クラスの `lock()`/`unlock()` および `basicDisconnect` の冒頭にデバッグログを追加して呼び出しスタックを確認し、`basicDisconnect` が呼ばれない原因を特定してから修正する。
