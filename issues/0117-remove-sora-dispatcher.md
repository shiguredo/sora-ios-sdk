...
- `0107` が完了し、consumer package と公開 API baseline が提供されていること。
...
- production code、test、consumer package、sample、documentation から参照を削除する。
...
- `0107` の consumer package の legacy scenario から `SoraDispatcher` の参照が削除され、consumer package が compile できることを確認する。
- `0070` の webrtc_c 移行が完了している場合は、移行後の code に generic な dispatch API が再導入されていないことを確認する。

## 完了条件

- `SoraDispatcher` の型と実装ファイルが削除されていること。
- production code、test、consumer package、documentation に参照が残っていないこと。
