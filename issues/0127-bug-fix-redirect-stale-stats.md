...
- 統計要求時に取得した `dataChannelGeneration` (transport epoch) と、コールバック発火時の現在の `dataChannelGeneration` を照合し、不一致の場合は送信しない。
- pong 送信時に `nativeChannel` (RTCPeerConnection) の identity を照合し、旧 PC の統計を新接続へ送信しない。
...
- production の transport epoch 管理へ redirect、ping、statistics callback のイベント列を入力し、epoch 不一致の統計送信が必ず拒否されることを検証する。
...
- 統計コールバック内で epoch 照合が行われ、不一致の場合に破棄されること。
