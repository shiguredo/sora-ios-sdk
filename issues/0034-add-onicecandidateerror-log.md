...
`Sora/PeerChannel.swift` の `didGenerate candidate:` と `didRemove candidates:` が実装されている。一方、ICE candidate の収集失敗を通知する `peerConnection(_:didFailToGatherIceCandidate:)` は実装しておらず、収集失敗時に何も記録されない。
...
- `Sora/PeerChannel.swift` の `RTCPeerConnectionDelegate` 実装に `peerConnection(_:didFailToGatherIceCandidate:)` を追加する。挿入位置は `didRemove candidates:` の直後（`didOpen dataChannel:` の前）。
- 既存の PC delegate（`didChange newState:` / `didGenerate candidate:` / `didOpen dataChannel:`）と同様に、`isCurrentPeerConnection(_:)` でリダイレクト中または旧 `RTCPeerConnection` からの遅延イベントを無視してからログ出力する。旧 PC の close 後に届く収集エラーをそのままログすると、リダイレクト後の接続のトラブルシュートで誤情報になるため。その際、`isCurrentPeerConnection` の doc コメント（本ヘルパーの適用対象の列挙）にも本メソッドを追記する。
