...
- 接続時カメラ起動と `setVideoHardMute` の可否判定が、実装時に選択した設計方針に一致していること
  - 譲渡を拒否する場合: 両方の経路が「別接続が使用中のカメラ」を拒否し、要求した側に拒否が通知されること
  - 譲渡を許す場合: 譲渡が仕様として文書化され、譲渡時に前の接続へ通知されること
- 譲渡が起きた場合に前の接続が検知できること (通知または `videoEnabled` の更新)
- 上記が型 doc と `skills/sora-ios-sdk/SKILL.md` に記載されていること
- 実機で再現手順を通し、選択した設計方針の期待どおりの結果になること
