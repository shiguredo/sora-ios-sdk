...
2. sendonly を接続する（`role: .sendonly`、`simulcastEnabled: true`、`videoEnabled: true`、`audioEnabled: false`、`videoCodec: .vp8`、`videoBitRate: 5000`、`initialCameraEnabled: false`）。`videoBitRate` は明示指定する（simulcast では帯域制御がレイヤーごとにビットレートを割り当てるため、無指定だと高解像度レイヤー (r2) に 0 が割り当てられエンコードされない。ビットレートは 3 レイヤーで分割して割り当てられるため、各レイヤーに十分な値が確保できるよう 5000 を指定する）。connect コールバックは実行キューが固定されていないため、共有状態の更新と後続処理は main queue に束ねる（`E2ETests` は @MainActor）
3. sendonly の connect コールバック内で `channel.senderStream` を取得して DummyVideoCapturer（640×480 @ 15fps）を設定・開始し、そのまま recvonly を r0 → r1 → r2 の順で接続する（`role: .recvonly`、`simulcastEnabled: true`、`simulcastRequestRid: .r0` / `.r1` / `.r2`、`videoEnabled: true`、`audioEnabled: false`、`videoCodec: .vp8`）。recvonly は送信しないため senderStream の guard は行わない。生成した capturer はテストメソッドスコープの変数に保持する（保持しないとコールバック終了時に解放され、フレーム送信が止まる）。接続した `MediaChannel` も同様に保持する
