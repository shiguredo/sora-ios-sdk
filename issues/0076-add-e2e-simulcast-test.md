...
2. sendonly を接続する（`role: .sendonly`、`simulcastEnabled: true`、`videoEnabled: true`、`audioEnabled: false`、`videoCodec: .vp8`、`videoBitRate: 1500`、`initialCameraEnabled: false`）。`videoBitRate` は js-sdk と同様に明示指定する（simulcast では帯域制御がレイヤーごとにビットレートを割り当てるため、無指定だと高解像度レイヤー (r2) に 0 が割り当てられエンコードされない）。connect コールバックは実行キューが固定されていないため、共有状態の更新と後続処理は main queue に束ねる（`E2ETests` は @MainActor）
