...
- `Any` から `JSONValue` への変換は `Sora/JSONValue.swift` に internal の `JSONValue.fromJSONObject(_:) throws -> JSONValue` を追加して行う。値がトップレベルで文字列・数値・bool・null の場合でも直列化できるよう、`fromDataChannels(_:errorReason:)` と同じ形で `JSONValueKey.value` の key を持つ辞書へ包み、`JSONSerialization.isValidJSONObject` で検証してから `JSONSerialization.data(withJSONObject:)` と `JSONDecoder().decode([String: JSONValue].self, from:)` で読む (`isValidJSONObject` はトップレベルの断片に対して false を返すため、包んでから検証する)。
...
- `init(code:message:errorObject:)` は変換に失敗した場合も `code` / `message` を失わず `data: nil` とし、英語のメッセージで `Logger.error(type: .dataChannel, ...)` を出す。この経路は `JSONSerialization.jsonObject` が返した値では到達しないが、テストからは `["data": Date()]` のような値で到達させられる。
...
- `from(_:errorReason:)` / `fromDataChannels(_:errorReason:)` / `fromJSONObject(_:)` は internal のまま公開しない。`errorReason` を引数に取る SDK 内部向けの変換を公開 API にしないためである。
...
- `Sora/JSONValue.swift`: `JSONValue` と `encode(to:)` / `init(from:)` の public 化、`/// :nodoc:` の削除、doc コメントの更新、internal な `fromJSONObject(_:)` の追加
- `SoraTests/RPCErrorDetailTests.swift`: internal な init と `fromJSONObject(_:)` の検証 (新規)
...
- `issues/0152-add-sendable-configuration-api.md`: `JSONValue` の public 化は本 issue が完了済みであることと、変換エラーの写像の分離は `0152` が扱うことへ更新
...
- `SoraTests/RPCErrorDetailTests.swift` で `JSONValue.fromJSONObject(_:)` を直接検証する。object / array / string / bool / 整数 / 小数 / `NSNull` が期待する `JSONValue` になり、`Date` のような JSON にできない値では `EncodingError.invalidValue` を throw すること
...
- `make consumer-build` のあと、`Sora/` の全ファイルを Swift 6 言語モードで型検査する。判定は件数の絶対値ではなく差分で行い、`associated value 'rpcServerError(detail:)'` の警告が消えて他の警告が増えていないことを確認する。2026-09-24 の実測は主診断の warning 行が 54 件 (参考値)。
  ...
  ```
- 実 Sora のテストサーバーで server error 応答を発生させ、`SoraError.rpcServerError(detail:)` として `code` / `message` が従来どおり取得できることと、返ってきた `data` を確認する。発生条件の候補は、テストが生成する JWT の private claim で制御できる `simulcast_rpc_rids` に含めない rid を `RequestSimulcastRid` で要求する方法と、`rpc_methods` に含まれないメソッド名を利用者定義の `RPCMethodProtocol` 準拠型で要求する方法である。この 2 つが server error 応答になることと `data` が付くことは一次情報で確認できていないため、実装時に実測する。server error を発生させられない場合、または `data` が付かない場合は、試した呼び出しと応答を `## 検証記録` に残し、`data` の JSON の検証は単体テストで担保する。
...
- `from(_:errorReason:)` / `fromDataChannels(_:errorReason:)` / `fromJSONObject(_:)` が internal のままで、公開 API に `errorReason` を引数に取る変換が現れていないこと
