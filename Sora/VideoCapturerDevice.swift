...
public enum VideoCapturerDevice {
    ...
    /**
     ...
     このオプションが使用されている場合、 `Sora` はストリームへの接続完了時、自動的にカメラを起動して動画のキャプチャと配信を開始します。
    ...
    /**
     ...
     したがってこのオプションを使用する場合は、ストリームへの接続完了後、自身でストリームの `VideoCapturer` を設定しない限り、動画は配信されません。
     ...
     - カメラ以外の動画ソースから動画のキャプチャと配信を行いたいとき。
     - 動画のキャプチャ開始・終了タイミングを細かく調整したいとき。
     */
    case custom
