...
final class TURNTLSCertificateVerifier: NSObject, RTCSSLCertificateVerifier {
  ...
  func verify(_ certificate: Data) -> Bool {
    guard let cert = SecCertificateCreateWithData(nil, certificate as CFData) else {
      Logger.debug(
        type: .peerChannel,
        message: "TURNTLSCertificateVerifier: Failed to create certificate from data")
      return false
    }
    ...
    guard status == errSecSuccess, let trust = trust else {
      Logger.debug(
        type: .peerChannel, message: "TURNTLSCertificateVerifier: Failed to create trust")
      return false
    }
    ...
    if !result {
      Logger.debug(
        type: .peerChannel,
        message:
          "TURNTLSCertificateVerifier: Certificate verification failed: \(error?.localizedDescription ?? "Unknown error")"
      )
    } else {
      Logger.debug(
        type: .peerChannel,
        message: "TURNTLSCertificateVerifier: Certificate verification succeeded")
    }
