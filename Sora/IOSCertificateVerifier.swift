...
final class IOSCertificateVerifier: NSObject, RTCSSLCertificateVerifier {
  ...
  private static func evaluate(_ certificateChain: [SecCertificate]) -> Bool {
    ...
    // ホスト名検証は BoringSSL 側で SSLPostConnectionCheck 内の
    // X509_check_host により TLS ハンドシェイク完了後に別途実行される。
    // 本メソッドは libwebrtc 組み込みの証明書検証と並行して動作し（OR 判定）、
    // iOS の Security フレームワークによる証明書チェーン検証を行う。
